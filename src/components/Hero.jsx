...
const Hero = ({  title, paragraph, formButton }) => {
  return (
      ...
      <div className="container mx-auto top h-fit ">
        <div className='row flex'>
          <div className='column sm:w-8/12 w-12/12 '>
            <h1 className='lg:text-7xl sm:text-6xl sm:mr-0  text-4xl font-primary-font font-bold text-[#253d4e]'>{title="Welcome to Shopper, Your personal Store"}</h1>
            {paragraph === null ? " " : (
            ...
            {formButton === null ? " " :
              (
                ...
                <form className='flex justify-between items-center pl-6 bg-white rounded-full sm:w-[70%] md:mt-16'>
                  ...
                  <button className='md:py-5 md:px-12 px-4 py-4 rounded-full bg-primary-color text-white hover:bg-primary-color-dark transition linear delay-150'>{formButton="submit"}</button>
                </form>
              )
            }
          </div>
